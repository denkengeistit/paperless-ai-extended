...
const consolidationService = require('../services/consolidationService.js');
...
// Add consolidation and summaries routes to public routes for navigation
PUBLIC_ROUTES.push('/consolidation', '/summaries');
...
/**
 * @swagger
 * /api/consolidation/similar-tags:
 *   get:
 *     summary: Find similar tags in the system
 *     description: |
 *       Find similar tags based on name similarity to help with tag consolidation.
 *       This endpoint uses Levenshtein distance algorithm to identify potentially duplicated tags.
 *     tags:
 *       - Metadata
 *       - API
 *     security:
 ...
 *           maximum: 1
 *           default: 0.7
 *         description: Similarity threshold (0-1), higher values require greater similarity
 *     responses:
 *       200:
 *         description: List of similar tag groups
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *         description: Server error
 */
router.get('/api/consolidation/similar-tags', protectApiRoute, async (req, res) => {
  try {
    const threshold = parseFloat(req.query.threshold) || 0.7;
    const similarGroups = await consolidationService.findSimilarTags(threshold);
    res.json({ success: true, similarGroups });
  } catch (error) {
...
/**
 * @swagger
 * /api/consolidation/similar-correspondents:
 *   get:
 *     summary: Find similar correspondents in the system
 *     description: |
 *       Find similar correspondents based on name similarity to help with correspondent consolidation.
 *       This endpoint uses Levenshtein distance algorithm to identify potentially duplicated correspondents.
 *     tags:
 *       - Metadata
 *       - API
 *     security:
 ...
 *     responses:
 *       200:
 *         description: List of similar correspondent groups
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *         description: Server error
 */
router.get('/api/consolidation/similar-correspondents', protectApiRoute, async (req, res) => {
  try {
    const threshold = parseFloat(req.query.threshold) || 0.7;
    const similarGroups = await consolidationService.findSimilarCorrespondents(threshold);
    res.json({ success: true, similarGroups });
  } catch (error) {
    console.error('[ERROR] finding similar correspondents:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
/**
 * @swagger
 * /api/consolidation/similar-document-types:
 *   get:
 *     summary: Find similar document types in the system
 *     description: |
 *       Find similar document types based on name similarity to help with document type consolidation.
 *       This endpoint uses Levenshtein distance algorithm to identify potentially duplicated document types.
 *     tags:
 *       - Metadata
 *       - API
 *     security:
 ...
 *     responses:
 *       200:
 *         description: List of similar document type groups
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *         description: Server error
 */
router.get('/api/consolidation/similar-document-types', protectApiRoute, async (req, res) => {
  try {
    const threshold = parseFloat(req.query.threshold) || 0.7;
    const similarGroups = await consolidationService.findSimilarDocumentTypes(threshold);
    res.json({ success: true, similarGroups });
  } catch (error) {
    console.error('[ERROR] finding similar document types:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
/**
 * @swagger
 * /api/consolidation/consolidate-tags:
 *   post:
 *     summary: Consolidate similar tags
 *     description: |
 *       Consolidate similar tags by redirecting all documents using tags-to-merge to the primary tag.
 *     tags:
 *       - Metadata
 *       - API
 *     security:
 ...
 *               primaryTagId:
 *                 type: integer
 *                 description: ID of the primary tag to keep
 *               tagsToMerge:
 *                 type: array
 *                 items:
 *                   type: integer
 *                 description: Array of tag IDs to merge into the primary tag
 *     responses:
 *       200:
 ...
 *         description: Server error
 */
router.post('/api/consolidation/consolidate-tags', protectApiRoute, async (req, res) => {
  try {
    const { primaryTagId, tagsToMerge } = req.body;
    
    if (!primaryTagId || !tagsToMerge || !Array.isArray(tagsToMerge) || tagsToMerge.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'Invalid input: primaryTagId and tagsToMerge array are required' 
      });
    }
    
    const result = await consolidationService.consolidateTags({ primaryTagId, tagsToMerge });
    res.json(result);
  } catch (error) {
...
/**
 * @swagger
 * /api/consolidation/consolidate-correspondents:
 *   post:
 *     summary: Consolidate similar correspondents
 *     description: |
 *       Consolidate similar correspondents by redirecting all documents using correspondents-to-merge to the primary correspondent.
 *     tags:
 *       - Metadata
 *       - API
 *     security:
 ...
 *             type: object
 *             required:
 *               - primaryCorrespondentId
 *               - correspondentsToMerge
 *             properties:
 *               primaryCorrespondentId:
 *                 type: integer
 *                 description: ID of the primary correspondent to keep
 *               correspondentsToMerge:
 *                 type: array
 *                 items:
 *                   type: integer
 *                 description: Array of correspondent IDs to merge into the primary correspondent
 *     responses:
 *       200:
 *         description: Consolidation results
 *       401:
 ...
 *         description: Server error
 */
router.post('/api/consolidation/consolidate-correspondents', protectApiRoute, async (req, res) => {
  try {
    const { primaryCorrespondentId, correspondentsToMerge } = req.body;
    
    if (!primaryCorrespondentId || !correspondentsToMerge || !Array.isArray(correspondentsToMerge) || correspondentsToMerge.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'Invalid input: primaryCorrespondentId and correspondentsToMerge array are required' 
      });
    }
    
    const result = await consolidationService.consolidateCorrespondents({ primaryCorrespondentId, correspondentsToMerge });
    res.json(result);
  } catch (error) {
    console.error('[ERROR] consolidating correspondents:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
/**
 * @swagger
 * /api/consolidation/consolidate-document-types:
 *   post:
 *     summary: Consolidate similar document types
 *     description: |
 *       Consolidate similar document types by redirecting all documents using document-types-to-merge to the primary document type.
 *     tags:
 *       - Metadata
 *       - API
 *     security:
 ...
 *               - documentTypesToMerge
 *             properties:
 *               primaryDocumentTypeId:
 *                 type: integer
 *                 description: ID of the primary document type to keep
 *               documentTypesToMerge:
 *                 type: array
 *                 items:
 *                   type: integer
 *                 description: Array of document type IDs to merge into the primary document type
 *     responses:
 *       200:
 *         description: Consolidation results
 *       401:
 ...
 *         description: Server error
 */
router.post('/api/consolidation/consolidate-document-types', protectApiRoute, async (req, res) => {
  try {
    const { primaryDocumentTypeId, documentTypesToMerge } = req.body;
    
    if (!primaryDocumentTypeId || !documentTypesToMerge || !Array.isArray(documentTypesToMerge) || documentTypesToMerge.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'Invalid input: primaryDocumentTypeId and documentTypesToMerge array are required' 
      });
    }
    
    const result = await consolidationService.consolidateDocumentTypes({ primaryDocumentTypeId, documentTypesToMerge });
    res.json(result);
  } catch (error) {
...
/**
 * @swagger
 * /api/summary/generate/{documentId}:
 *   post:
 *     summary: Generate and save a document summary
 *     description: |
 *       Generate a summary for the specified document and save it to the Notes field in PaperlessNGX.
 *     tags:
 *       - Documents
 *       - API
 *     security:
 ...
 *         description: Server error
 */
router.post('/api/summary/generate/:documentId', protectApiRoute, async (req, res) => {
  try {
 ...
 * /api/summary/batch:
 *   post:
 *     summary: Generate summaries for multiple documents
 *     description: |
 *       Generate summaries for multiple documents and save them to the Notes field in PaperlessNGX.
 *     tags:
 *       - Documents
 *       - API
 *     security:
 ...
 *         description: Server error
 */
router.post('/api/summary/batch', protectApiRoute, async (req, res) => {
  try {
    ...
    if (!documentIds || !Array.isArray(documentIds) || documentIds.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'Invalid input: documentIds array is required' 
      });
    }
    ...
    console.error('[ERROR] batch processing summaries:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
 ...
 * /consolidation:
 *   get:
 *     summary: Render the metadata consolidation tool page
 *     description: |
 *       Provides a UI for comparing and consolidating similar tags, correspondents, and document types.
 *     tags:
 *       - Navigation
 *       - Metadata
 *     security:
 ...
 *     responses:
 *       200:
 *         description: Metadata consolidation tool page rendered successfully
 *       401:
 *         description: Unauthorized - user not authenticated
 *       500:
 *         description: Server error
 */
router.get('/consolidation', authenticateJWT, async (req, res) => {
  try {
    res.render('consolidation', { 
      page: 'consolidation',
      user: req.user,
      config: configFile
    });
  } catch (error) {
    console.error('[ERROR] rendering consolidation page:', error);
 ...
 * /summaries:
 *   get:
 *     summary: Render the document summaries tool page
 *     description: |
 *       Provides a UI for generating and managing AI summaries of documents.
 *     tags:
 *       - Navigation
 *       - Documents
 *     security:
 ...
 *     responses:
 *       200:
 *         description: Document summaries tool page rendered successfully
 *       401:
 *         description: Unauthorized - user not authenticated
 *       500:
 *         description: Server error
 */
router.get('/summaries', authenticateJWT, async (req, res) => {
  try {
    ...
    console.error('[ERROR] rendering summaries page:', error);
    res.status(500).send('Server error');
  }
});
